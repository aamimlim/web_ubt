const quizMetadata = [
  {
    text: "[1-2] 다음 질문에 답하십시오.",
    num:"1.  다음 단어와 관계있는 것은 무엇입니까? ",
    detail:"빨갛다,노랗다,파랗다 하얗다",
    image: "",
    options: [
      { text: "산" },
      { text: "몸" },
      { text: "색" },
      { text: "맛" }
    ],
    correct: 2,
    points: 2.5
  },
 {
    text: "[1-2] 다음 질문에 답하십시오.",
    num:"2. 다음 단어의 비슷한 말을 무엇입니까? ",
    detail:"(화상을) 입다",
    image: "",
    options: [
      { text: "데다" },
      { text: "식하다" },
      { text: "박히다" },
      { text: "베이다" }
    ],
    correct: 0,
    points: 2.5
  },
 {
    text: "[3-6] 빈 칸에 들어갈 가장 알맞은 것을 고르십시오.",
    num:"3.",
    detail:"수박 한 통에 ______예요?  ",
    image: "",
    options: [
      { text: "누구" },
      { text: "언제" },
      { text: "어디" },
      { text: "얼마" }
    ],
    correct: 3,
    points: 2.5
  },
 {
    text: "[3-6] 빈 칸에 들어갈 가장 알맞은 것을 고르십시오.",
    num:"4.",
    detail:"휴게실은 ________장소 입니다. 다른 사람을 배려해서 사용합시다. ",
    image: "",
    options: [
      { text: "금지" },
      { text: "공공 " },
      { text: "무단 " },
      { text: "종교" }
    ],
    correct: 2,
    points: 2.5
  },
 {
    text: "[3-6] 빈 칸에 들어갈 가장 알맞은 것을 고르십시오.",
    num:"5.",
    detail:"아침에 일어나니까 _________가 많이 아팠습니다. 그래서 약국에 가서 두통약을 사 먹었습니다.",
    image: "",
    options: [
      { text: "이" },
      { text: "배" },
      { text: "머리" },
      { text: "다리" }
    ],
    correct: 2,
    points: 2.5
  },
 {
    text: "[3-6] 빈 칸에 들어갈 가장 알맞은 것을 고르십시오.",
    num:"6.",
    detail:"출퇴근 시간에는 버스보다 지하철이 ______ 빠릅니다. 도로에는 차가 많아서 복잡하기 때문입니다.",
    image: "",
    options: [
      { text: "더" },
      { text: "잘" },
      { text: "참" },
      { text: "약" }
    ],
    correct: 0,
    points: 2.5
  },
 {
    text: "[7-10] 다음 질문에 답하십시오.",
    num:"7. 이 표지는 무슨 뜻입니까?  ",
    detail:"",
    image: "https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8_7.webp",
    options: [
      { text: "올라탈 수 없습니다. " },
      { text: "걸을 수 없습니다. " },
      { text: "이동할 수 없습니다. " },
      { text: "들어올 수 없습니다." }
    ],
    correct: 0,
    points: 2.5
  },
 {
    text: "[7-10] 다음 질문에 답하십시오.",
    num:"8.  다음은 약 봉투 입니다. 맞는 것을 고르십시오. ",
    detail:"",
    image: "https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8_8.webp",
    options: [
      { text: "약사는 김지수입니다." },
      { text: "약은 열흘 분입니다." },
      { text: "식사 후에 약을 먹습니다." },
      { text: "다친 곳에 바르는 약입니다." }
    ],
    correct: 2,
    points: 2.5
  },
 {
    text: "[7-10] 다음 질문에 답하십시오.",
    num:"9.  다음 안내문에 대한 설명으로 맞는 것은 무엇입니까? ",
    detail:"",
    image: "https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8_9.webp",
    options: [
      { text: "행복편의점 옆에 미래약국이 있습니다." },
      { text: "미래약국이 편의점 옆으로 이사갑니다." },
      { text: "오늘은 문화약국에서 약을 살 수 있습니다." },
      { text: "미래약국은 한 달에 한 번 쉽니다." }
    ],
    correct: 2,
    points: 2.5
  },
 {
    text: "[7-10] 다음 질문에 답하십시오.",
    num:"10. 직장인들이 어떻게 스트레스를 푸는지 조사했습니다. 책을 읽으면서 스트레스를 푸는 사람은 얼마나 됩니까? ",
    detail:"",
    image: "https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8_10.webp",
    options: [
      { text: "8%" },
      { text: "15%" },
      { text: "19%" },
      { text: "23%" }
    ],
    correct: 1,
    points: 2.5
  },
 {
    text: "[11-12] 빈칸에 들어갈 가장 알맞은 것을 고르십시오.",
    num:"11.",
    detail:"코, 귀, 목이 아프면 ______로 가야합니다.",
    image: "",
    options: [
      { text: "치과" },
      { text: "이비인후과" },
      { text: "안과" },
      { text: "정형외과" }
    ],
    correct: 1,
    points: 2.5
  },
 {
    text: "[11-12] 빈칸에 들어갈 가장 알맞은 것을 고르십시오.",
    num:"12.",
    detail:"여름에는 논에 농약을 치고_______ 을/를 뽑는 작업을 계속 해야 합니 다. ",
    image: "",
    options: [
      { text: "잡초" },
      { text: "분뇨" },
      { text: "거름" },
      { text: "씨앗" }
    ],
    correct: 0,
    points: 2.5
  },
 {
    text: "[13-14] 다음 글을 읽고 무엇에 대한 글인지 고르십시오.",
    num:"13.",
    detail:"한국에서는 이사를 가거나 결혼을 하면 가족이나 친구를 초대합니다. 집주인은 손님들에게 집을 보여 주고 음식을 대접합니다. 초대를 받은 사람은 선물을 사 가는데 보통 휴지나 세제를 많이 선물합니다. 이런 선물들에는 일이 잘되라는 의미가 있습니다.",
    image: "",
    options: [
      { text: "돌잔지" },
      { text: "피로연" },
      { text: "집들이" },
      { text: "상견례" }
    ],
    correct: 2,
    points: 2.5
  },
 {
    text: "[13-14] 다음 글을 읽고 무엇에 대한 글인지 고르십시오.",
    num:"14.",
    detail:"저희 할머니께서는 자주 편찮으셨습니다. 할머니께서 편찮으실 때마다 마음이 많이 아팠습니다. 그래서 저는 어릴 때부터 의사가 되고 싶었습니다. 지금은 의사가 되려고 대학교에서 열심히 공부하고 있습니다. 공부하는 것은 힘들지만 마음은 즐겁습니다. 꼭 촣은 의사가 되고 아픈 사람들을 도와 주겠습니다.",
    image: "",
    options: [
      { text: "꿈" },
      { text: "운" },
      { text: "비난" },
      { text: "태도" }
    ],
    correct: 0,
    points: 2.5
  },
 {
    text: "[15-16] 다음 글을 읽고 내용과 같은 것을 고르십시오.",
    num:"15.",
    detail:"여름에 과도한 에어컨의 사용으로 냉방병에 걸리는 경우가 많습니다. 여 름철 냉방병을 예방하기 위해서 실내외 온도차를 5!C 정도로 유지해야 합 니다. 가벼운 긴팔 옷을 준비해서 체온을 유지하는 것도 중요합니다. 매시 간마다 에어컨을 끄고 환기를 해 주어야 합니다. 이 것을 지키면 냉방병을 예방할 수 있을 것입니다.",
    image: "",
    options: [
      { text: "냉방병의 원인은 에어컨의 과다 사용입니다. " },
      { text: "실내외 온도차는 많을수록 좋습니다" },
      { text: "체온을 유지하기 위해서 난방을 해야 합니다. " },
      { text: "십분마다 에어컨을 끄고 환기를 해 줍니다." }
    ],
    correct: 0,
    points: 2.5
  },
 {
    text: "[15-16] 다음 글을 읽고 내용과 같은 것을 고르십시오.",
    num:"16.",
    detail:"민수씨 고맙습니다. 탁자의 상태가 좋지 않아서 여러 번 사포질을 해야 해서 팔이 너무 아팠어요. 그런데 민수씨가 새로 산 연마기를 빌려주어 서 쉽게 금방 끝낼 수 있었어요. 기계가 좋아서 탁자가 아주 매끄럽게 되었네요. 너무 고마워요. 다음에 도움이 필요하면 언제든지 부탁하세 요. 그때는 제가 꼭 도와드릴게요. ",
    image: "",
    options: [
      { text: "민수씨는 팔이 너무 아팠습니다." },
      { text: " 민수씨가 연마기를 빌려주었습니다. " },
      { text: "민수씨가 탁자를 여러 번 사포질했습니다. " },
      { text: "민수씨는 언제든지 도움을 요청합니다." }
    ],
    correct: 1,
    points: 2.5
  },
 {
    text: "[17-18] 다음 설명에 알맞은 어휘를 고르십시오.",
    num:"17.",
    detail:"소, 닭 등 가축들의 질병을 예방하고 병을 치료하는 직업입니다 .",
    image: "",
    options: [
      { text: "상담사" },
      { text: "수의사" },
      { text: "목수" },
      { text: "어부" }
    ],
    correct: 1,
    points: 2.5
  },
 {
    text: "[17-18] 다음 설명에 알맞은 어휘를 고르십시오.",
    num:"18.",
    detail:"현재의 체류 기간이 만료하기 전 2개월부터 만료 당일까지 신청 해야 합니다. 체류 기간 만료일이 지난 후 체류 기간 연장 허가를 신청하게 되면 법칙금을 부과됩니다.",
    image: "",
    options: [
      { text: "체류 기간 연장 허가 신청 이유" },
      { text: "체류 기간 연장 허가 신청 방법" },
      { text: "체류 기간 연장 허가 신청 기간" },
      { text: "체류 기간 연장 허가 신청 중요성" }
    ],
    correct: 2,
    points: 2.5
  },
 {
    text: "[19-20] 다음 그림을 보고 맞는 단어나 문장을 고르십시오.",
    num:"19.",
    detail:"",
    image: "https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8_19.webp",
    options: [
      { text: "벌 " },
      { text: "곰 " },
      { text: "나비" },
      { text: "매미" }
    ],
    correct: 2,
    points: 2.5
  },
 {
    text: "[19-20] 다음 그림을 보고 맞는 단어나 문장을 고르십시오.",
    num:"20.",
    detail:"",
    image: "https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8_20.webp",
    options: [
      { text: "도장을 하고 있습니다. " },
      { text: "포장을 하고 있습니다. " },
      { text: "분류를 하고 있습니다. " },
      { text: "용접을 하고 있습니다." }
    ],
    correct: 2,
    points: 2.5
  },
 {
    text: "[21-24] 들은 것을 고르십시오.",
    num:"21.",
    detail:"",
    image: "",
    audio:"https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8-21.mp3",
    options: [
      { text: "가울" },
      { text: "겨울" },
      { text: "거리" },
      { text: "아들" }
    ],
    correct: 1,
    points: 2.5
  },
 {
    text: "[21-24] 들은 것을 고르십시오.",
    num:"22.",
    detail:"",
    image: "",
    audio:"https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8-22.mp3",
    options: [
      { text: "벽지 " },
      { text: "벽장 " },
      { text: "벽해 " },
      { text: "벽돌" }
    ],
    correct: 3,
    points: 2.5
  },
 {
    text: "[21-24] 들은 것을 고르십시오.",
    num:"23.",
    detail:"",
    image: "https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8_23.webp",
    audio:"https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8-23.mp3",
    options: [
      { text: "" },
      { text: "" },
      { text: "" },
      { text: "" }
    ],
    correct: 0,
    points: 2.5
  },
 {
    text: "[21-24] 들은 것을 고르십시오.",
    num:"24.",
    detail:"",
    image: "https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8_24.webp",
    audio:"https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8-24.mp3",
    options: [
      { text: "" },
      { text: "" },
      { text: "" },
      { text: "" }
    ],
    correct: 2,
    points: 2.5
  },
 {
    text: "[25-29] 그림을 보고 알맞은 대답을 고르십시오.",
    num:"25. 이것은 무엇입니까?",
    detail:"",
    image: "https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8_25.webp",
    audio:"https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8-25.mp3",
    options: [
      { text: "" },
      { text: "" },
      { text: "" },
      { text: "" }
    ],
    correct: 1,
    points: 2.5
  },
 {
    text: "[25-29] 그림을 보고 알맞은 대답을 고르십시오.",
    num:"26. 여기는 어디입니까? ",
    detail:"",
    image: "https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8_26.webp",
    audio:"https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8-26.mp3",
    options: [
      { text: "" },
      { text: "" },
      { text: "" },
      { text: "" }
    ],
    correct: 2,
    points: 2.5
  },
 {
    text: "[25-29] 그림을 보고 알맞은 대답을 고르십시오.",
    num:"27. 이 사람은 무엇을 하고 있습니까?",
    detail:"",
    image: "https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8_27.webp",
    audio:"https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8-27.mp3",
    options: [
      { text: "" },
      { text: "" },
      { text: "" },
      { text: "" }
    ],
    correct: 2,
    points: 2.5
  },

 {
    text: "[25-29] 그림을 보고 알맞은 대답을 고르십시오.",
    num:"28. 한복이 얼마나 있습니까? ",
    detail:"",
    image: "https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8_28.webp",
    audio:"https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8-28.mp3",
    options: [
      { text: "" },
      { text: "" },
      { text: "" },
      { text: "" }
    ],
    correct: 0,
    points: 2.5
  },
 {
    text: "[25-29] 그림을 보고 알맞은 대답을 고르십시오.",
    num:"29. 이불 가게는 어디에 있습니까?",
    detail:"",
    image: "https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8_29.webp",
    audio:"https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8-29.mp3",
    options: [
      { text: "" },
      { text: "" },
      { text: "" },
      { text: "" }
    ],
    correct: 3,
    points: 2.5
  },
 {
    text: "[30-33] 다음을 듣고 질문에 알맞은 대답을 고르십시오.",
    num:"30.",
    detail:"",
    image: "",
    audio:"https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8-30.mp3",
    options: [
      { text: "시청앞에서 보는게 어때요? " },
      { text: "좋아요, 같이 점심 먹어요. " },
      { text: "회사앞에서 만났어요. " },
      { text: "매일 도서관에서 친구를 만나요." }
    ],
    correct: 0,
    points: 2.5
  },
 {
    text: "[30-33] 다음을 듣고 질문에 알맞은 대답을 고르십시오.",
    num:"31.",
    detail:"",
    image: "",
    audio:"https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8-31.mp3",
    options: [
      { text: "이백사십이요. " },
      { text: "이백 킬로미터예요. " },
      { text: "삼백 그램이에요. " },
      { text: "중자입니다." }
    ],
    correct: 0,
    points: 2.5
  },
 {
    text: "[30-33] 다음을 듣고 질문에 알맞은 대답을 고르십시오.",
    num:"32.",
    detail:"",
    image: "",
    audio:"https://jettyland.wordpress.com/wp-content/uploads/2025/01/g5_set1-32.mp3",
    options: [
      { text: "석유를 많이 수입해요. " },
      { text: "저는 컴퓨터 게임을 많이 해요. " },
      { text: "휴대폰하고 텔레비전이에요. " },
      { text: "경제가 안 좋아진 것 같아요." }
    ],
    correct: 2,
    points: 2.5
  },
 {
    text: "[30-33] 다음을 듣고 질문에 알맞은 대답을 고르십시오.",
    num:"33.",
    detail:"",
    image: "",
    audio:"https://jettyland.wordpress.com/wp-content/uploads/2025/01/g5_set1-33.mp3",
    options: [
      { text: "네, 그렇게 하세요. " },
      { text: "네, 제가 사올 게요. " },
      { text: "아니요, 저에게 주세요. " },
      { text: "아니요, 지금 사오세요." }
    ],
    correct: 0,
    points: 2.5
  },
 {
    text: "[34-35] 다음을 듣고 이어지는 말을 고르십시오.",
    num:"34.",
    detail:"",
    image: "",
    audio:"https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8-34.mp3",
    options: [
      { text: "네, 번호대로 서 있을게요. " },
      { text: "네, 그런데 번호표는 어디에 있나요? " },
      { text: "아니요, 제 대기 번호가 더 나중이에요. " },
      { text: "아니요, 대기 번호가 화면에 뜨면 창구로 가세요." }
    ],
    correct: 1,
    points: 2.5
  },
 {
    text: "[34-35] 다음을 듣고 이어지는 말을 고르십시오.",
    num:"35.",
    detail:"",
    image: "",
    audio:"https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8-35.mp3",
    options: [
      { text: "외출하기 좋은 날이네요. " },
      { text: "주말이라 좀 복잡하네요. " },
      { text: "맞아요. 정말 화창한데요" },
      { text: "어떡하죠? 우산이 없는데" }
    ],
    correct: 3,
    points: 2.5
  },
 {
    text: "[36-37] 다음을 듣고 들은 내용과 관계 있는 그림을 고르십시오.",
    num:"36.",
    detail:"",
    image: "",
    audio:"https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8-36.mp3",
    options: [
      { text: "" },
      { text: "" },
      { text: "" },
      { text: "" }
    ],
    correct: 1,
    points: 2.5
  },
 {
    text: "[36-37] 다음을 듣고 들은 내용과 관계 있는 그림을 고르십시오.",
    num:"37.",
    detail:"",
    image: "",
    audio:"https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8-37.mp3",
    options: [
      { text: "" },
      { text: "" },
      { text: "" },
      { text: "" }
    ],
    correct: 2,
    points: 2.5
  },
 {
    text: "[38-40] 다음 이야기를 듣고 질문에 알맞은 대답을 고르십시오.",
    num:"38. 두사람은 어디에서 만납니까? ",
    detail:"",
    image: "",
    audio:"https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8-38.mp3",
    options: [
      { text: "회사 앞에서  " },
      { text: "지방에서" },
      { text: "시청 앞에서 " },
      { text: "식당에서" }
    ],
    correct: 2,
    points: 2.5
  },
 {
    text: "[38-40] 다음 이야기를 듣고 질문에 알맞은 대답을 고르십시오.",
    num:"39. 남자는 왜 명절에 고향에 내려가지 않습니까?  ",
    detail:"",
    image: "",
    audio:"https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8-39.mp3",
    options: [
      { text: "부모님이 안 계셔서 " },
      { text: "부모님이 서울로 오셔서 " },
      { text: "명절에 교통이 막혀서 " },
      { text: "기차표 예매를 못해서" }
    ],
    correct: 1,
    points: 2.5
  },
 {
    text: "[38-40] 다음 이야기를 듣고 질문에 알맞은 대답을 고르십시오.",
    num:"40. 어떤 경우에 공장을 옮길 수 있습니까? ",
    detail:"",
    image: "",
    audio:"https://jettyland.wordpress.com/wp-content/uploads/2025/01/g4_set8-40.mp3",
    options: [
      { text: "공장이 폐업했을 때" },
      { text: "공장이 이사를 했을 때" },
      { text: "근무 환경이 안 좋을 때" },
      { text: "공장 일이 너무 힘들 때" }
    ],
    correct: 0,
    points: 2.5
  }
];