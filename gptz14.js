const quizMetadata = [


  {
    text:"[1~4] 다음 그림을 보고 알맞은 것을 고르십시오.",
    num:"1.",
    detail:"작업자가 전동 드릴을 사용하고 있습니다.",
    image:"set17-q1.png",
    audio:"",
    options:[
      { text:"전동 드릴을 사용하고 있습니다." },
      { text:"자재를 손으로 옮기고 있습니다." },
      { text:"기계를 청소하고 있습니다." },
      { text:"제품을 포장하고 있습니다." }
    ],
    correct:0,
    points:2.5
  },

  {
    text:"[1~4] 다음 그림을 보고 알맞은 것을 고르십시오.",
    num:"2.",
    detail:"직원이 서류를 복사하고 있습니다.",
    image:"set17-q2.png",
    audio:"",
    options:[
      { text:"서류를 복사하고 있습니다." },
      { text:"서류를 폐기하고 있습니다." },
      { text:"서류를 우편으로 보내고 있습니다." },
      { text:"서류를 번역하고 있습니다." }
    ],
    correct:0,
    points:2.5
  },

  {
    text:"[1~4] 다음 그림을 보고 알맞은 것을 고르십시오.",
    num:"3.",
    detail:"한 남자가 지하철 승강장에서 교통카드를 충전하고 있습니다.",
    image:"set17-q3.png",
    audio:"",
    options:[
      { text:"교통카드를 충전하고 있습니다." },
      { text:"표를 환불하고 있습니다." },
      { text:"지하철을 청소하고 있습니다." },
      { text:"택시를 기다리고 있습니다." }
    ],
    correct:0,
    points:2.5
  },

  {
    text:"[1~4] 다음 그림을 보고 알맞은 것을 고르십시오.",
    num:"4.",
    detail:"농부가 비닐하우스에서 작물에 물을 주고 있습니다.",
    image:"set17-q4.png",
    audio:"",
    options:[
      { text:"작물에 물을 주고 있습니다." },
      { text:"작물을 포장하고 있습니다." },
      { text:"농기계를 수리하고 있습니다." },
      { text:"비닐하우스를 청소하고 있습니다." }
    ],
    correct:0,
    points:2.5
  },


  {
    text:"[5~6] 다음 중 알맞은 것을 고르십시오.",
    num:"5.",
    detail:"회의가 길어져서 점심시간이 평소보다 ______ 시작되었습니다.",
    image:"",
    audio:"",
    options:[
      { text:"늦게" },
      { text:"늦도록" },
      { text:"늦으면" },
      { text:"늦어서" }
    ],
    correct:0,
    points:2.5
  },

  {
    text:"[5~6] 다음 중 알맞은 것을 고르십시오.",
    num:"6.",
    detail:"이 약은 식사한 후에 물과 함께 ______ 합니다.",
    image:"",
    audio:"",
    options:[
      { text:"복용해야" },
      { text:"복용하면" },
      { text:"복용하거나" },
      { text:"복용해서" }
    ],
    correct:0,
    points:2.5
  },


  {
    text:"[7~10] 다음 글을 읽고 물음에 답하십시오.",
    num:"7. 주차장 정기 청소는 언제 실시됩니까?",
    detail:"",
    image:"set17-q7.png",
    audio:"",
    options:[
      { text:"월요일 오전" },
      { text:"화요일 오후" },
      { text:"수요일 오전" },
      { text:"금요일 오후" }
    ],
    correct:2,
    points:2.5
  },

  {
    text:"[7~10] 다음 글을 읽고 물음에 답하십시오.",
    num:"8. 무인 민원 발급기를 이용할 때 필요한 것은 무엇입니까?",
    detail:"",
    image:"set17-q8.png",
    audio:"",
    options:[
      { text:"사원증" },
      { text:"신분증" },
      { text:"통장" },
      { text:"사진 두 장" }
    ],
    correct:1,
    points:2.5
  },

  {
    text:"[7~10] 다음 글을 읽고 물음에 답하십시오.",
    num:"9. 사내 식당 저녁 식사는 몇 시부터 시작합니까?",
    detail:"",
    image:"set17-q9.png",
    audio:"",
    options:[
      { text:"오후 4시 30분" },
      { text:"오후 5시" },
      { text:"오후 5시 30분" },
      { text:"오후 6시" }
    ],
    correct:2,
    points:2.5
  },

  {
    text:"[7~10] 다음 글을 읽고 물음에 답하십시오.",
    num:"10. 반납한 공구는 어디에 두어야 합니까?",
    detail:"",
    image:"set17-q10.png",
    audio:"",
    options:[
      { text:"작업대 위" },
      { text:"개인 사물함" },
      { text:"공구 보관함" },
      { text:"창고 입구" }
    ],
    correct:2,
    points:2.5
  },


  {
    text:"[11~16] 다음 문장의 빈칸에 들어갈 가장 알맞은 것을 고르십시오.",
    num:"11.",
    detail:"예약한 날짜를 변경하려면 담당 부서에 미리 ______ 합니다.",
    image:"",
    audio:"",
    options:[
      { text:"연락해야" },
      { text:"연락하면" },
      { text:"연락하거나" },
      { text:"연락해서" }
    ],
    correct:0,
    points:2.5
  },

  {
    text:"[11~16] 다음 문장의 빈칸에 들어갈 가장 알맞은 것을 고르십시오.",
    num:"12.",
    detail:"작업 지시서를 확인한 뒤 필요한 자재를 ______ 가져왔습니다.",
    image:"",
    audio:"",
    options:[
      { text:"따로" },
      { text:"벌써" },
      { text:"아직" },
      { text:"서로" }
    ],
    correct:0,
    points:2.5
  },

  {
    text:"[11~16] 다음 문장의 빈칸에 들어갈 가장 알맞은 것을 고르십시오.",
    num:"13.",
    detail:"이 기계는 고장이 자주 나기 때문에 정기적으로 상태를 ______ 해야 합니다.",
    image:"",
    audio:"",
    options:[
      { text:"점검" },
      { text:"점검하면" },
      { text:"점검하고" },
      { text:"점검해서" }
    ],
    correct:0,
    points:2.5
  },

  {
    text:"[11~16] 다음 문장의 빈칸에 들어갈 가장 알맞은 것을 고르십시오.",
    num:"14.",
    detail:"비밀번호를 다른 사람에게 알려 주지 ______ 주의해야 합니다.",
    image:"",
    audio:"",
    options:[
      { text:"않도록" },
      { text:"않으면" },
      { text:"않거나" },
      { text:"않아서" }
    ],
    correct:0,
    points:2.5
  },

  {
    text:"[11~16] 다음 문장의 빈칸에 들어갈 가장 알맞은 것을 고르십시오.",
    num:"15.",
    detail:"회의에 늦지 않으려고 평소보다 30분 ______ 출발했습니다.",
    image:"",
    audio:"",
    options:[
      { text:"일찍" },
      { text:"일찍이" },
      { text:"이른" },
      { text:"일찍은" }
    ],
    correct:0,
    points:2.5
  },

  {
    text:"[11~16] 다음 문장의 빈칸에 들어갈 가장 알맞은 것을 고르십시오.",
    num:"16.",
    detail:"계약 내용을 이해하기 어려우면 서명하기 전에 담당자에게 ______ 것이 좋습니다.",
    image:"",
    audio:"",
    options:[
      { text:"확인하는" },
      { text:"확인한" },
      { text:"확인할" },
      { text:"확인해서" }
    ],
    correct:0,
    points:2.5
  },



  {
    text:"다음 글을 읽고 물음에 답하십시오.",
    num:"17. 이 안내의 목적으로 알맞은 것은 무엇입니까?",
    detail:"택배 보관함 이용 안내\n택배가 도착하면 문자로 보관함 번호와 인증번호가 전송됩니다. 보관함 앞 화면에 인증번호를 입력한 후 물건을 찾아가십시오. 보관 기간은 도착일로부터 3일이며, 기간이 지나면 물건이 관리실로 이동됩니다.",
    image:"set17-q17.png",
    audio:"",
    options:[
      { text:"택배를 보내는 방법을 안내하려고" },
      { text:"택배를 찾는 방법과 보관 기간을 안내하려고" },
      { text:"택배 요금을 안내하려고" },
      { text:"관리실 위치를 안내하려고" }
    ],
    correct:1,
    points:2.5
  },

  {
    text:"다음 글을 읽고 물음에 답하십시오.",
    num:"18. 지하철 정기권은 어떻게 충전할 수 있습니까?",
    detail:"지하철 정기권 충전 안내\n정기권은 역 안의 충전기에서 충전할 수 있습니다. 충전할 때는 기존 정기권을 충전기에 넣고 원하는 금액을 선택한 후 결제하십시오. 현금과 카드 모두 사용할 수 있습니다.",
    image:"set17-q18.png",
    audio:"",
    options:[
      { text:"충전기에서 현금이나 카드로 충전할 수 있습니다." },
      { text:"버스 안에서만 충전할 수 있습니다." },
      { text:"은행에서만 충전할 수 있습니다." },
      { text:"현금으로만 충전할 수 있습니다." }
    ],
    correct:0,
    points:2.5
  },

  {
    text:"[19~20] 다음 글을 읽고 내용과 같은 것을 고르십시오.",
    num:"19.",
    detail:"지훈 씨는 회사에서 새로 사용하는 재고 관리 프로그램을 배우고 있습니다. 처음에는 입력 방법이 복잡했지만, 매일 연습한 덕분에 지금은 입고 수량과 출고 수량을 혼자 입력할 수 있습니다.",
    image:"",
    audio:"",
    options:[
      { text:"지훈 씨는 프로그램을 사용하지 못합니다." },
      { text:"지훈 씨는 매일 프로그램을 연습하고 있습니다." },
      { text:"지훈 씨는 입고 수량만 입력할 수 있습니다." },
      { text:"지훈 씨는 회사에서 프로그램을 가르칩니다." }
    ],
    correct:1,
    points:2.5
  },

  {
    text:"[19~20] 다음 글을 읽고 내용과 같은 것을 고르십시오.",
    num:"20.",
    detail:"이번 달부터 회사에서는 전기 절약을 위해 퇴근 전에 컴퓨터와 조명을 확인하는 제도를 시행합니다. 마지막으로 퇴근하는 직원은 사용하지 않는 전자기기의 전원을 끄고 조명을 확인해야 합니다.",
    image:"",
    audio:"",
    options:[
      { text:"모든 직원이 아침마다 조명을 확인합니다." },
      { text:"전기 절약 제도는 다음 달부터 시작합니다." },
      { text:"마지막 퇴근자는 전자기기와 조명을 확인해야 합니다." },
      { text:"사용하지 않는 전자기기는 계속 켜 두어야 합니다." }
    ],
    correct:2,
    points:2.5
  },

  {
    text:"[21~22] 들은 것을 고르십시오.",
    num:"21.",
    detail:"",
    image:"",
    audio:"audio/set17/q21.mp3",
    options:[
      { text:"반품" },
      { text:"반문" },
      { text:"방문" },
      { text:"발품" }
    ],
    correct:0,
    points:2.5
  },

  {
    text:"[21~22] 들은 것을 고르십시오.",
    num:"22.",
    detail:"",
    image:"",
    audio:"audio/set17/q22.mp3",
    options:[
      { text:"재고" },
      { text:"제고" },
      { text:"제공" },
      { text:"제곱" }
    ],
    correct:0,
    points:2.5
  },


  {
    text:"[23~28] 듣고 알맞은 그림을 고르십시오.",
    num:"23.",
    detail:"",
    image:"",
    audio:"audio/set17/q23.mp3",
    options:[
      { image:"set17-q23-1.png" },
      { image:"set17-q23-2.png" },
      { image:"set17-q23-3.png" },
      { image:"set17-q23-4.png" }
    ],
    correct:2,
    points:2.5
  },

  {
    text:"[23~28] 듣고 알맞은 그림을 고르십시오.",
    num:"24.",
    detail:"",
    image:"",
    audio:"audio/set17/q24.mp3",
    options:[
      { image:"set17-q24-1.png" },
      { image:"set17-q24-2.png" },
      { image:"set17-q24-3.png" },
      { image:"set17-q24-4.png" }
    ],
    correct:1,
    points:2.5
  },

  {
    text:"[23~28] 듣고 알맞은 그림을 고르십시오.",
    num:"25.",
    detail:"",
    image:"",
    audio:"audio/set17/q25.mp3",
    options:[
      { image:"set17-q25-1.png" },
      { image:"set17-q25-2.png" },
      { image:"set17-q25-3.png" },
      { image:"set17-q25-4.png" }
    ],
    correct:1,
    points:2.5
  },

  {
    text:"[23~28] 듣고 알맞은 그림을 고르십시오.",
    num:"26.",
    detail:"",
    image:"",
    audio:"audio/set17/q26.mp3",
    options:[
      { image:"set17-q26-1.png" },
      { image:"set17-q26-2.png" },
      { image:"set17-q26-3.png" },
      { image:"set17-q26-4.png" }
    ],
    correct:2,
    points:2.5
  },

  {
    text:"[23~28] 듣고 알맞은 그림을 고르십시오.",
    num:"27.",
    detail:"",
    image:"",
    audio:"audio/set17/q27.mp3",
    options:[
      { image:"set17-q27-1.png" },
      { image:"set17-q27-2.png" },
      { image:"set17-q27-3.png" },
      { image:"set17-q27-4.png" }
    ],
    correct:1,
    points:2.5
  },

  {
    text:"[23~28] 듣고 알맞은 그림을 고르십시오.",
    num:"28.",
    detail:"",
    image:"",
    audio:"audio/set17/q28.mp3",
    options:[
      { image:"set17-q28-1.png" },
      { image:"set17-q28-2.png" },
      { image:"set17-q28-3.png" },
      { image:"set17-q28-4.png" }
    ],
    correct:0,
    points:2.5
  },


  {
    text:"[29~32] 듣고 알맞은 대답을 고르십시오.",
    num:"29.",
    detail:"",
    image:"",
    audio:"audio/set17/q29.mp3",
    options:[
      { text:"네, 회의 자료를 다시 보내 드리겠습니다." },
      { text:"네, 지난주에 회의가 있었습니다." },
      { text:"아니요, 회의실은 3층입니다." },
      { text:"오후 두 시부터 근무합니다." }
    ],
    correct:0,
    points:2.5
  },

  {
    text:"[29~32] 듣고 알맞은 대답을 고르십시오.",
    num:"30.",
    detail:"",
    image:"",
    audio:"audio/set17/q30.mp3",
    options:[
      { text:"네, 이쪽에서 기다리겠습니다." },
      { text:"아니요, 아직 신청하지 않았습니다." },
      { text:"네, 여권을 복사했습니다." },
      { text:"오전 10시에 도착했습니다." }
    ],
    correct:1,
    points:2.5
  },

  {
    text:"[29~32] 듣고 알맞은 대답을 고르십시오.",
    num:"31.",
    detail:"",
    image:"",
    audio:"audio/set17/q31.mp3",
    options:[
      { text:"네, 이 서류를 먼저 작성하겠습니다." },
      { text:"아니요, 서류가 책상 위에 있습니다." },
      { text:"다음 주에 휴가를 갑니다." },
      { text:"은행은 오후 4시에 문을 닫습니다." }
    ],
    correct:0,
    points:2.5
  },

  {
    text:"[29~32] 듣고 알맞은 대답을 고르십시오.",
    num:"32.",
    detail:"",
    image:"",
    audio:"audio/set17/q32.mp3",
    options:[
      { text:"네, 창고에서 가져왔습니다." },
      { text:"오후 세 시에 출발했습니다." },
      { text:"다음 달부터 사용합니다." },
      { text:"아니요, 오늘은 회의가 없습니다." }
    ],
    correct:2,
    points:2.5
  },


  {
    text:"[33] 이어지는 말을 고르십시오.",
    num:"33.",
    detail:"",
    image:"",
    audio:"audio/set17/q33.mp3",
    options:[
      { text:"그럼 출발하기 전에 한 번 더 확인해 보겠습니다." },
      { text:"네, 어제 시장에 다녀왔습니다." },
      { text:"오후에는 비가 올 것 같습니다." },
      { text:"아니요, 지하철을 타고 왔습니다." }
    ],
    correct:0,
    points:2.5
  },


  {
    text:"[34~36] 듣고 알맞은 그림을 고르십시오.",
    num:"34.",
    detail:"",
    image:"",
    audio:"audio/set17/q34.mp3",
    options:[
      { image:"set17-q34-1.png" },
      { image:"set17-q34-2.png" },
      { image:"set17-q34-3.png" },
      { image:"set17-q34-4.png" }
    ],
    correct:0,
    points:2.5
  },

  {
    text:"[34~36] 듣고 알맞은 그림을 고르십시오.",
    num:"35.",
    detail:"",
    image:"",
    audio:"audio/set17/q35.mp3",
    options:[
      { image:"set17-q35-1.png" },
      { image:"set17-q35-2.png" },
      { image:"set17-q35-3.png" },
      { image:"set17-q35-4.png" }
    ],
    correct:0,
    points:2.5
  },

  {
    text:"[34~36] 듣고 알맞은 그림을 고르십시오.",
    num:"36.",
    detail:"",
    image:"",
    audio:"audio/set17/q36.mp3",
    options:[
      { image:"set17-q36-1.png" },
      { image:"set17-q36-2.png" },
      { image:"set17-q36-3.png" },
      { image:"set17-q36-4.png" }
    ],
    correct:0,
    points:2.5
  },


  {
    text:"[37~40] 듣고 물음에 답하십시오.",
    num:"37. 여자는 무엇을 준비하려고 합니까?",
    detail:"",
    image:"",
    audio:"audio/set17/q37.mp3",
    options:[
      { text:"출장 서류" },
      { text:"기념품" },
      { text:"저녁 식사" },
      { text:"작업복" }
    ],
    correct:0,
    points:2.5
  },

  {
    text:"[37~40] 듣고 물음에 답하십시오.",
    num:"38. 남자는 왜 창구에 가려고 합니까?",
    detail:"",
    image:"",
    audio:"audio/set17/q38.mp3",
    options:[
      { text:"통장을 만들려고" },
      { text:"분실한 카드를 신고하려고" },
      { text:"현금을 찾으려고" },
      { text:"환율을 확인하려고" }
    ],
    correct:1,
    points:2.5
  },

  {
    text:"[37~40] 듣고 물음에 답하십시오.",
    num:"39. 여자는 어디에서 물건을 받을 예정입니까?",
    detail:"",
    image:"",
    audio:"audio/set17/q39.mp3",
    options:[
      { text:"회사 정문" },
      { text:"지하철역" },
      { text:"택배 보관함" },
      { text:"주민센터" }
    ],
    correct:2,
    points:2.5
  },

  {
    text:"[37~40] 듣고 물음에 답하십시오.",
    num:"40. 남자는 주말에 무엇을 하려고 합니까?",
    detail:"",
    image:"",
    audio:"audio/set17/q40.mp3",
    options:[
      { text:"낚시를 하려고" },
      { text:"도서관에 가려고" },
      { text:"자전거를 수리하려고" },
      { text:"가족과 박물관에 가려고" }
    ],
    correct:3,
    points:2.5
  }

];